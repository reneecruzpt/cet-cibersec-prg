...
class Bullet(GameSprite):
    def __init__(self, picture, player_x, player_y, size_x, size_y, speed):
        super().__init__(picture, player_x, player_y, size_x, size_y)
        self.speed = speed

    def update(self):
        self.rect.x += self.speed
        #desaparece ao atingir o limite do ecrã
...
#criando um inimigo
enemy = Enemy('enemy.png',630,200,50,50,2)

#criando um grupo de inimigos
enemies = sprite.Group()

#adicionando inimigos ao grupo
enemies.add(enemy)
...
#loop runs every 0.05 seconds
    ...
    if finish != True:
        ...
        enemies.draw(window)
        final.reset()
        bullets.draw(window) #desenha as balas na tela

        #verificando colisões de grupos
        sprite.groupcollide(enemies, bullets, True, True) #inimigos e projéteis
        sprite.groupcollide(bullets, walls, True, False) #projéteis e paredes
        ...
        #     finish = True
        if sprite.spritecollide(packman, enemies, False):
            finish = True
    else:
        if victory == True:
            window.blit(win,(0,0))
